...
class NLP:
    ...
    def analyze(self, text: str) -> Doc:
        ...
        >>> cltk_nlp = NLP(language="lat")
        >>> cltk_obj = cltk_nlp.analyze(text=LAT)
        ...
        >>> cltk_obj = cltk_nlp.analyze(text=GRC)
        >>> cltk_obj.words[0]
        ...
        >>> from cltkv1.utils.example_texts import CHU
        >>> cltk_nlp = NLP(language="chu")
        >>> cltk_obj = cltk_nlp.analyze(text=CHU)
        >>> cltk_obj.words[0]
        ...
        >>> cltk_obj = cltk_nlp.analyze(text=FRO)
        >>> cltk_obj.words[0]
        ...
        >>> from cltkv1.utils.example_texts import GOT
        >>> cltk_nlp = NLP(language="got")
        >>> cltk_obj = cltk_nlp.analyze(text=GOT)
        >>> cltk_obj.words[0]
